...
export const env = {
  ...
  storageLimitGb: Number(process.env.STORAGE_LIMIT_GB ?? 1),
  maxFileSizeMb: Number(process.env.MAX_FILE_SIZE_MB ?? 25),

  supabase: {
    url: process.env.SUPABASE_URL ?? "",
    serviceRoleKey: process.env.SUPABASE_SERVICE_ROLE_KEY ?? "",
    bucket: (process.env.SUPABASE_BUCKET ?? "Documents").trim(),
    enabled: Boolean(process.env.SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY),
  },

  ai: {
    provider: (process.env.AI_PROVIDER ?? "ollama") as "ollama" | "gemini" | "openai",
